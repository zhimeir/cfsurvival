#' Confidence interval based on random forest (scaled)
#'
...
rf_scaled <- function(x,r,
  ...
  mdl <- crf.km(fmla, ntree = ntree, 
                 ...
                 tau = 0.25,
                 method = "grf")
  weight_lo <- mdl$predicted[1:n_calib]
  new_weight_lo <- tail(mdl$predicted,-n_calib)
  ...
  mdl <- crf.km(fmla, ntree = ntree, 
                 ...
                 tau = 0.75,
                 method = "grf")
  weight_up <- mdl$predicted[1:n_calib]
  new_weight_up <- tail(mdl$predicted,-n_calib)

  weight <- weight_up - weight_lo
  new_weight <- new_weight_up - new_weight_lo
